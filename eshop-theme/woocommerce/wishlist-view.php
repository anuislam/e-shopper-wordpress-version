...
<form id="yith-wcwl-form" action="<?php echo $form_action ?>" method="post" class="woocommerce">
    ...
    if( ! empty( $page_title ) ) :
    ?>
            <?php echo apply_filters( 'yith_wcwl_wishlist_title', '<h2  class="title text-center" style="margin:20px 0;">' . $page_title . '</h2>' ); ?>
    ...
    <!-- WISHLIST TABLE -->
	<table class="shop_table cart wishlist_table" id="eshop_wishlist" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo $wishlist_id ?>" data-token="<?php echo $wishlist_token ?>">
        ...
        <tr class="cart_menu ">
            ...
            <th class="product-thumbnail text_center"></th>

            <th class="product-name text_center">
            ...
            <?php if( $show_price ) : ?>
                ...
                <th class="product-price text_center">
                    <span class="nobr">
            ...
            <?php if( $show_stock_status ) : ?>
                ...
                <th class="product-stock-status text_center">
                    <span class="nobr">
            ...
            <?php if( $show_last_column ) : ?>
                ...
                <th class="product-add-to-cart text_center"></th>
            ...
            <?php if( $is_user_owner ): ?>
                <th class="product-remove text_center"></th>
            <?php
                $column_count ++;
            endif;
            ?>
                ...
                if( $product && $product->exists() ) :
                    ...
                    <tr class="cart_item" id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>">
                        ...
                        <td class="product-thumbnail">
                            ...
                            <?php
$cart_image = wp_get_attachment_image_src( $product->get_image_id(), 'eshop_cart_image' );
echo  '<img src="'.$cart_image[0].'" alt="'.$product->get_name().'">';
                            ?>
                        ...
                        <td class="cart_description" style="text-align:left;">
                            ...
                            <h4><a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>">
                                <?php echo apply_filters( 'woocommerce_in_cartproduct_obj_title', $product->get_title(), $product ) ?></a>
                            </h4>
                            <p>Web ID: <?php echo $product->get_sku(); ?></p>
                             <?php do_action( 'yith_wcwl_table_after_product_name', $item ); ?>
                        </td>

                        <?php if( $show_price ) : ?>
                            <td class="cart_price text_center">
                                <p>
                        ...
                        <?php if( $show_stock_status ) : ?>
                            <td class="cart_description text_center">
                                <p>
	                ...
	                <?php endif; ?>
                        ...
                        <td class="cart_delete text_center">
                            <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist cart_quantity_delete" title="<?php _e( 'Remove this product', 'yith-woocommerce-wishlist' ) ?>" style="display:inline-block;margin: 40px 10px;"><i class="fa fa-times"></i></a>
                        </td>
                        <?php endif; ?>
